...
function ExitScreen({ type, onBack }: ScanScreenProps) {
  ...
  const handleTakeSnapshot = useCallback(() => {
    ...
    if (viewId) {
      UIManager.dispatchViewManagerCommand(viewId, 'takeSnapshot', []);
    }
  ...
  const handleRetake = useCallback(() => {
    ...
    const viewId = findNodeHandle(cameraRef.current);
    if (viewId) {
      UIManager.dispatchViewManagerCommand(viewId, 'resetSnapshot', []);
    }
  }, []);
  ...
  const handleConfirm = useCallback(() => {
    if (plateText.trim()) {
      console.log(`Đã ghi nhận xe ra: ${plateText}`);
      handleRetake();
    }
